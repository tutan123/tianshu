...
(() => {
  ...
  function renderMap() {
    ...
    $('event-list').innerHTML = sides.length ? sides.map(id => `<button class="event-row" data-event="${id}">${icon('message-square')}<span><strong>${esc(CONTENT.nodes[id].title)}</strong><small>${esc(CONTENT.places[CONTENT.nodes[id].place].name)} · 支线事件</small></span></button>`).join('') : `<p class="event-empty">${state.done.length ? '暂时没有新的约定。<br>继续前行，故事正在发生。' : '熟悉的校园，新的开始。<br>有人正在宿舍等你。'}</p>`;
  ...
  function interaction() {
    ...
    $('encounter').innerHTML = `<div class="eyebrow">${icon('git-branch')} 命运分歧</div><h2>${esc(n.question)}</h2><p>天枢等待你的决定。</p>${n.choices.map((c, i) => {
      const { locked, poor } = TS.choiceState(state, a.id, i);
      return `<button class="choice" data-choice="${i}" ${locked || poor ? 'disabled' : ''}><span class="choice-index">0${i + 1}</span><span><strong>${esc(c.text)}</strong><small>${locked ? esc(c.lockedHint || '尚未取得所需证据') : poor ? '算力不足 · 需要 ' + c.cost : esc(c.hint)}</small></span>${icon(locked ? 'lock-keyhole' : 'arrow-up-right')}</button>`;
    }).join('')}`;
  ...
  function showResult() {
    ...
    $('interaction').innerHTML = `<div class="encounter"><div class="result-seal">${icon(a.outcome === 'perfect' ? 'sparkles' : 'check')}</div><div class="eyebrow">${a.outcome === 'perfect' ? '精准命中' : a.outcome === 'fail' ? '另一种前进' : '命运已发生偏移'}</div><h2>${esc(n.title)}</h2><p class="result-text">${esc(a.text)}</p><div class="reward-list">${rewards.map(r => `<span>${esc(r)}</span>`).join('')}</div><button class="primary" id="back-campus">${a.id === 'finale' ? '查看我的首章结局' : '回到校园'}${icon('arrow-right')}</button></div>`;
  ...
  function panelBody() {
    ...
    if (panelName === 'people') body.innerHTML = `<p class="panel-intro">有些关系，不在最优解里。<br>羁绊达到 20，解锁一段不曾说出口的往事。</p><div class="people-grid">${Object.entries(CONTENT.people).map(([id, p]) => `<article class="person"><div class="person-head"><div class="avatar" style="color:${p.color};${p.image ? 'background-image:url(' + p.image + ')' : ''}">${p.image ? '' : esc(p.initial)}</div><div><h3>${esc(p.name)}</h3><div class="role">${esc(p.role)}</div></div></div><p>${esc(p.bio)}</p><div class="bond-info"><span>${state.bonds[id] >= 35 ? '默契相知' : state.bonds[id] >= 20 ? '渐渐靠近' : state.bonds[id] ? '初识' : '尚待相遇'}</span><span>${state.bonds[id]} / 100</span></div><div class="bond-track"><span style="width:${state.bonds[id]}%;background:${p.color}"></span></div><p class="secret ${state.bonds[id] < 20 ? 'locked' : ''}">${state.bonds[id] >= 20 ? esc(p.secret) : '一段尚未解锁的往事 · 羁绊 20'}</p></article>`).join('')}</div>`;
    if (panelName === 'phone') {
      body.innerHTML = state.messages.length ? [...state.messages].reverse().map(m => {
        ...
        return `<article class="thread"><div class="thread-head"><div class="avatar small" style="color:${p.color}">${esc(p.initial)}</div><div>${esc(p.name)}<small>${m.read ? '已读' : '新消息'}</small></div></div><p class="chat-bubble">${esc(data.text)}</p>${m.reply !== null ? `<p class="chat-bubble self">${esc(data.replies[m.reply].text)}</p><p class="chat-bubble">${esc(data.replies[m.reply].response)}</p>` : `<div class="reply-row">${data.replies.map((r, i) => `<button class="secondary" data-reply="${m.id}:${i}">${esc(r.text)}</button>`).join('')}</div>`}</article>`;
    ...
    if (panelName === 'timeline') {
      body.innerHTML = `<p class="panel-intro">已改写 ${state.done.length} / ${Object.keys(CONTENT.nodes).length} 段命运 · 首章结局 ${state.ending ? '1' : '0'} / 3</p>${state.ending ? `<button class="secondary" id="ending-open">${icon('award')} ${esc(state.ending.title)}</button><br><br>` : ''}<div class="timeline">${CONTENT.main.map(id => {
        const n = CONTENT.nodes[id], done = state.done.includes(id), current = TS.available(state).includes(id), c = n.choices?.[state.choices[id]];
        return `<div class="timeline-item ${done ? 'complete' : current ? '' : 'locked'}"><h3>${esc(done || current ? n.title : '尚未发生的故事')}</h3><small>${esc(n.subtitle)} · ${esc(CONTENT.places[n.place].name)}</small>${done ? `<p>${c ? '你选择了：' + esc(c.text) : state.results[id] === 'fail' ? '你找到了另一种前进的方式。' : '你完成了这次挑战。'}</p><button class="secondary" data-review="${id}">${icon('history')} 回顾这一幕</button>` : current ? '<p>当前命运节点</p>' : ''}</div>`;
      }).join('')}</div>`;
    ...
    if (panelName === 'saves') {
      body.innerHTML = `<p class="panel-intro">当前进度：第 ${state.day} 日 · ${state.done.length} 段故事已记录。<br>自动存档会在剧情推进、选择与战术行动后更新。</p>${[1, 2, 3].map(i => {
        ...
        return `<div class="save-row"><span class="save-number">0${i}</span><div class="save-detail"><h3>${s ? s.ending?.title || (s.active ? esc(CONTENT.nodes[s.active.id].title) : '第 ' + s.day + ' 日 · 校园') : '空白记忆'}</h3><p>${s ? s.done.length + ' 段故事 · ' + (s.savedAt ? new Date(s.savedAt).toLocaleString('zh-CN') : '') : '尚未写入记录'}</p></div><button class="secondary" data-save="${i}">写入</button><button class="secondary" data-load="${i}" ${s ? '' : 'disabled'}>读取</button></div>`;
